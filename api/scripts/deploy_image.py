...
def main():
    parser = argparse.ArgumentParser(description="Deploy Docker image to registry and/or environment.")
    parser.add_argument("--tag", required=True, help="Docker image tag to deploy")
    parser.add_argument("--target-tag", required=True, help="Target image tag (e.g., registry/repo:tag)")
    parser.add_argument("--project-id", default="unknown", help="Project ID for logging")
    parser.add_argument("--version-id", default="unknown", help="Version ID for logging")
    parser.add_argument("--label", action="append", help="Label to add to the container (key=value). Can be specified multiple times.")
    ...
    parser.add_argument("--run", action="store_true", help="Run the container after push with labels/network")
    parser.add_argument("--domain", help="Base domain for Traefik label generation (optional)")
    parser.add_argument("--hackathon-id", help="Hackathon ID for Traefik label generation (optional)")
    parser.add_argument("--username", help="Username for Traefik label generation (optional)")
    ...
    print(output)
    if rc != 0:
        logger.log_error(ScriptError(f"Image tag failed: {args.tag} -> {args.target_tag}"))
        sys.exit(rc)
