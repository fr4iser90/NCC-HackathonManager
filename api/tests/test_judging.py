...
def created_team_for_judging(
    ...
    team = (
        ...
        .filter(TeamModel.id == uuid.UUID(team_data["id"]))
        .first()
    )
...
def created_project_for_judging(
    ...
    response = client.post(
        "/projects/", headers=auth_headers_for_regular_user, json=project_payload
    )
    assert response.status_code == status.HTTP_201_CREATED, response.json()
    project_data = response.json()
    project = (
        db_session.query(ProjectModel)
        .filter(ProjectModel.id == uuid.UUID(project_data["id"]))
        .first()
    )
...
def created_criterion(
    ...
    data = response.json()
    criterion = (
        db_session.query(CriterionModel)
        .filter(CriterionModel.id == uuid.UUID(data["id"]))
        .first()
    )
...
def test_list_criteria_unauthenticated(
    client: TestClient, created_criterion: CriterionModel
):
...
def test_list_criteria_authenticated(
    client: TestClient,
    auth_headers_for_admin_user: Dict[str, str],
    created_criterion: CriterionModel,
):
    response = client.get(
        "/judging/criteria/", headers=auth_headers_for_admin_user
...
def test_delete_criterion_as_admin(
    ...
    response = client.delete(
        f"/judging/criteria/{criterion_id_to_delete}",
        headers=auth_headers_for_admin_user,
    )
    assert response.status_code == status.HTTP_204_NO_CONTENT
    assert (
        db_session.query(CriterionModel)
        .filter(CriterionModel.id == criterion_id_to_delete)
        .first()
        is None
    )
...
def test_delete_criterion_as_judge_forbidden(
    ...
    response = client.delete(
        f"/judging/criteria/{created_criterion.id}", headers=auth_headers_for_judge_user
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN
...
@pytest.fixture(scope="function")
def score_payload(
    created_project_for_judging: ProjectModel, created_criterion: CriterionModel
) -> Dict[str, Any]:
    return {
...
def test_submit_score_as_judge(
    ...
    response = client.post(
        "/judging/scores/", headers=auth_headers_for_judge_user, json=score_payload
    )
    assert response.status_code == status.HTTP_201_CREATED
    data = response.json()
    ...
    assert (
        db_session.query(ScoreModel)
        .filter(ScoreModel.id == uuid.UUID(data["id"]))
        .first()
        is not None
    )
...
def test_submit_score_as_regular_user_forbidden(
    ...
    response = client.post(
        "/judging/scores/", headers=auth_headers_for_regular_user, json=score_payload
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN
...
def test_submit_score_project_not_found(
    ...
    response = client.post(
        "/judging/scores/", headers=auth_headers_for_judge_user, json=payload
    )
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_submit_score_invalid_score_value(
    ...
    response_high = client.post(
        "/judging/scores/", headers=auth_headers_for_judge_user, json=payload_too_high
    )
    assert response_high.status_code == status.HTTP_400_BAD_REQUEST
    assert (
        f"Score must be between 0 and {created_criterion.max_score}"
        in response_high.json()["detail"]
    )
    ...
    response_low = client.post(
        "/judging/scores/", headers=auth_headers_for_judge_user, json=payload_too_low
    )
    assert response_low.status_code == status.HTTP_400_BAD_REQUEST
    assert (
        f"Score must be between 0 and {created_criterion.max_score}"
        in response_low.json()["detail"]
    )
...
def test_submit_score_duplicate_judge_project_criterion(
    ...
    assert (
        "judge has already scored this project for this criterion"
...
def test_update_score_as_submitter_judge(
    ...
    update_payload = ScoreUpdate(
        score=created_criterion.max_score - 1, comment="Updated notes by judge."
    )
    response = client.put(
...
def test_update_score_as_different_judge_forbidden(
    ...
    auth_headers_for_admin_user: Dict[
        str, str
    ],  # To get headers for a "different" judge
    ...
    client.post(
        ...
        json={
            ...
            "username": other_user_email.split("@")[0],
            "password": other_user_pass,
            "full_name": "Other User",
        },
    )
    login_resp = client.post(
        "/users/login", data={"email": other_user_email, "password": other_user_pass}
    )
    auth_headers_for_other_user = {
...
def test_update_score_not_found(
    client: TestClient, auth_headers_for_judge_user: Dict[str, str]
):
    non_existent_id = uuid.uuid4()
...
def test_list_scores_for_project_no_scores(
    client: TestClient,
    created_project_for_judging: ProjectModel,
    auth_headers_for_regular_user: Dict[str, str],
):
    ...
    team_resp = client.post(
        "/teams/",
        json={"name": f"Team_EmptyScoreList_{uuid.uuid4()}", "description": "Test"},
        headers=auth_headers_for_regular_user,
    )
    ...
    project_resp = client.post(
        "/projects/",
        json={
            "name": f"Project_EmptyScoreList_{uuid.uuid4()}",
            "team_id": new_team_id,
            "hackathon_id": str(created_project_for_judging.hackathon_id),
        },
        headers=auth_headers_for_regular_user,
    )
    if "id" not in project_resp.json():
        pytest.skip(
            f"Project creation failed or no 'id' in response: {project_resp.json()}"
        )
...
def test_list_scores_by_judge_self(
    ...
    response = client.get(
        f"/judging/scores/judge/{judge_id}", headers=auth_headers_for_judge_user
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert isinstance(data, list)
    assert len(data) >= 1
    assert any(
        s["id"] == str(created_score_by_judge.id) and s["judge_id"] == judge_id
        for s in data
    )
...
def test_list_scores_by_judge_as_admin_for_another_judge(
    ...
    response = client.get(
        f"/judging/scores/judge/{target_judge_id}", headers=auth_headers_for_admin_user
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert isinstance(data, list)
    assert len(data) >= 1
    assert any(
        s["id"] == str(created_score_by_judge.id) and s["judge_id"] == target_judge_id
        for s in data
    )
...
def test_list_scores_by_judge_as_different_judge_forbidden(
    ...
    auth_headers_for_admin_user: Dict[
        str, str
    ],  # Simulating a "different judge" for now.
    ...
    response = client.get(
        f"/judging/scores/judge/{target_judge_id}",
        headers=auth_headers_for_regular_user,
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN
...
def test_list_scores_by_judge_non_existent_judge_id(
    client: TestClient,
    auth_headers_for_admin_user: Dict[str, str],  # Admin can try to get any judge
):
    non_existent_judge_id = uuid.uuid4()
    response = client.get(
        f"/judging/scores/judge/{non_existent_judge_id}",
        headers=auth_headers_for_admin_user,
    )
    assert (
        response.status_code == status.HTTP_200_OK
    )  # Returns empty list if judge or scores not found
    assert response.json() == []
