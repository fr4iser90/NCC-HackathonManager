...
def test_admin_assign_and_remove_roles(client, admin_token, participant_id):
    ...
    # Assign judge role
    r = client.post(
        f"/users/{participant_id}/roles", headers=headers, json={"role": "judge"}
    )
    assert r.status_code in (200, 201), r.text
    # Assign mentor role
    r = client.post(
        f"/users/{participant_id}/roles", headers=headers, json={"role": "mentor"}
    )
    assert r.status_code in (200, 201), r.text
...
def test_participant_rights_update(
    client, admin_token, participant_token, participant_id
):
    admin_headers = {"Authorization": f"Bearer {admin_token}"}
    user_headers = {"Authorization": f"Bearer {participant_token}"}
    # Assign judge role
    r = client.post(
        f"/users/{participant_id}/roles", headers=admin_headers, json={"role": "judge"}
    )
