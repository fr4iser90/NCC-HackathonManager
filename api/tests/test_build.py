...
def test_upload_invalid_zip(client, test_hackathon, auth_headers_for_regular_user):
    ...
    data = {"project_id": str(project_id)}
    res = client.post(f"/projects/{project_id}/submit_version", files=files, data=data, headers=auth_headers_for_regular_user)
    assert res.status_code in (400, 422), f"Expected failure, got {res.status_code}: {res.text}"

@pytest.mark.usefixtures("client", "db_session", "test_hackathon", "created_regular_user", "auth_headers_for_regular_user")
def test_upload_zip_missing_project_files(client, test_hackathon, auth_headers_for_regular_user):
...
def test_build_multiple_versions(client, test_hackathon, auth_headers_for_regular_user):
    ...
    data = {"project_id": str(project_id)}
    res1 = client.post(f"/projects/{project_id}/submit_version", files=files1, data=data, headers=auth_headers_for_regular_user)
    ...
    res2 = client.post(f"/projects/{project_id}/submit_version", files=files2, data=data, headers=auth_headers_for_regular_user)
