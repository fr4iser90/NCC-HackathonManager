...
def test_user_registration(
    client: TestClient, db_session: Session, unique_id: uuid.UUID
):
...
def test_user_registration_duplicate_email(client: TestClient, unique_id: uuid.UUID):
    ...
    client.post(
        "/users/register",
        json={
            "email": email_for_duplicate_test,
            "password": "testpass",
    ...
    response = client.post(
        ...
        json={
            ...
            "username": f"another_dup_{unique_id}",
...
def test_user_login(client: TestClient, regular_user_data: dict):
    ...
    login_response = client.post(
        "/users/login", data={"email": user["email"], "password": user["password"]}
    )
...
def test_user_login_wrong_password(client: TestClient, regular_user_data: dict):
    ...
    user = regular_user_data
    login_response = client.post(
        "/users/login", data={"email": user["email"], "password": "thisisverywrong"}
    )
    assert login_response.status_code == status.HTTP_401_UNAUTHORIZED
    assert login_response.json()["detail"] == "Invalid credentials"
...
def test_get_me(
    client: TestClient, auth_headers_for_regular_user: dict, regular_user_data: dict
):
...
def test_update_another_user_profile_as_regular_user(
    ...
    target_user_id = admin_user_data[
        "id"
    ]  # Using admin_user_data just to get another user's ID
...
def test_update_another_user_profile_as_admin(
    ...
    db_user = (
        db_session.query(UserModel)
        .filter(UserModel.id == uuid.UUID(target_user_id))
        .first()
    )
    assert db_user.full_name == new_full_name
...
def test_update_profile_username_conflict(
    ...
    if "detail" in data:
        if isinstance(data["detail"], str):
            ...
            assert any(
                "Username already taken" in str(item) or "max_length" in str(item)
                for item in data["detail"]
            )


def test_update_profile_non_existent_user(
    client: TestClient, auth_headers_for_admin_user: dict
):
