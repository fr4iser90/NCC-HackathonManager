...
def test_judge_login_and_judging_flow(admin_user_data):
    ...
    r = httpx.post(
        f"{BASE_URL}/users/{judge_id}/roles",
    ...
    r = httpx.post(
        f"{BASE_URL}/users/login",
        data={"email": unique_email, "password": judge_password},
    )
    ...
    r = httpx.post(
        f"{BASE_URL}/hackathons/", headers=admin_headers, json=hackathon_data
    )
    ...
    r = httpx.post(
        f"{BASE_URL}/judging/criteria/",
        headers=headers,
        json={"name": "Test", "description": "desc", "max_score": 10, "weight": 1.0},
    )
    ...
    assert (
        isinstance(projects, list) and projects
    ), f"No projects found! Response: {r.text}"
    project_id = projects[0]["id"]
    r = httpx.get(f"{BASE_URL}/judging/criteria/", headers=headers)
    criteria = r.json() if r.status_code == 200 else []
    assert (
        isinstance(criteria, list) and criteria
    ), f"No criteria found! Response: {r.text}"
