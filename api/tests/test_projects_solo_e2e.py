...
def test_admin_crud_project_e2e(admin_user_data):
    ...
    if hackathons[0].get("status") != "active":
        patch = {"status": "active"}
        r2 = httpx.put(
            f"{BASE_URL}/hackathons/{hackathon_id}", json=patch, headers=headers
        )
    ...
    update_data = {
        "name": "E2E Admin Project Updated",
        "description": "Updated by admin",
    }
    r = httpx.put(
        f"{BASE_URL}/projects/{project_id}", json=update_data, headers=headers
    )
    assert r.status_code == 200, f"Project update failed: {r.text}"
    assert r.json()["name"] == "E2E Admin Project Updated"
...
def test_user_crud_project_e2e(admin_user_data):
    ...
    # 2. Login
    r = httpx.post(
        f"{BASE_URL}/users/login", data={"email": user_email, "password": user_password}
    )
    assert r.status_code == 200, f"Login failed: {r.text}"
    token = r.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
    ...
    if hackathon.get("status") != "active":
        ...
        patch = {"status": "active"}
        r2 = httpx.put(
            f"{BASE_URL}/hackathons/{hackathon_id}", json=patch, headers=admin_headers
        )
        assert r2.status_code in (200, 204), f"Failed to activate hackathon: {r2.text}"

    # --- SOLO HACKATHON: Projekt ohne Team ---
    project_data = {
        "name": "E2E User Solo Project",
        "description": "E2E User Solo Test",
        "hackathon_id": hackathon_id,
        "status": "active",
        "storage_type": "github",
    }
    r = httpx.post(f"{BASE_URL}/projects/", json=project_data, headers=headers)
    assert r.status_code == 201, f"Solo project creation failed: {r.text}"
    solo_project_id = r.json()["id"]
    # Bearbeiten
    update_data = {
        "name": "E2E User Solo Project Updated",
        "description": "Updated by user",
    }
    r = httpx.put(
        f"{BASE_URL}/projects/{solo_project_id}", json=update_data, headers=headers
    )
    assert r.status_code == 200, f"Solo project update failed: {r.text}"
    # Löschen
    r = httpx.delete(f"{BASE_URL}/projects/{solo_project_id}", headers=headers)
    assert r.status_code == 204, f"Solo project delete failed: {r.text}"
    ...
    r = httpx.put(
        f"{BASE_URL}/projects/{fremd_id}", json={"name": "Hacked"}, headers=headers
    )
    assert r.status_code in (
        403,
        404,
    ), f"User konnte fremdes Projekt bearbeiten! {r.text}"
...
def test_solo_project_forbidden_when_hackathon_not_active(admin_user_data):
    # Setup: Register and login user
    unique = str(uuid.uuid4())[:8]
    user_email = f"forbiddenuser_{unique}@example.com"
    user_password = "forbiddenpass123"
    r = httpx.post(
        f"{BASE_URL}/users/register",
        json={
            "email": user_email,
            "username": f"forbiddenuser_{unique}",
            "password": user_password,
    ...
    # Create project
    project_data = {
        "name": f"Forbidden Solo Project {unique}",
    ...
    r = httpx.post(
        f"{BASE_URL}/projects/{project_id}/submit_version", headers=headers, files=files
    )
...
def main():
    ...
    test_solo_project_forbidden_when_hackathon_not_active()
    print("E2E Solo Project Forbidden Test erfolgreich!")
