...
EXAMPLE_ZIP = os.path.join(
    os.path.dirname(__file__), "example_projects", "SimpleSecCheck-main.zip"
)
...
def test_build_logs_e2e(admin_user_data):
    ...
    with open(EXAMPLE_ZIP, "rb") as f:
        ...
        r = httpx.post(
            f"{BASE_URL}/projects/{project_id}/submit_version",
            files=files,
            headers=headers,
        )
        ...
        if r.status_code == 400:
            assert (
                "Hackathon is not active" in r.text
            ), f"Expected error message 'Hackathon is not active', got: {r.text}"
    ...
    assert any(
        v["id"] == version_id for v in versions
    ), f"Uploaded version {version_id} not found in project versions!"

    # 5. Build-Logs abfragen
    r = httpx.get(
        f"{BASE_URL}/projects/{project_id}/versions/{version_id}/build_logs",
        headers=headers,
    )
    if r.status_code == 200:
        ...
        assert (
            "Starte Build" in logs or "Build erfolgreich" in logs
        ), "Build logs do not contain expected output!"
    else:
        print(
            "Build-Log-Endpoint liefert 404, prüfe Build-Logs direkt aus Version-Liste!"
        )
...
def test_upload_invalid_zip_e2e(admin_user_data):
    ...
    r = httpx.post(
        f"{BASE_URL}/projects/{project_id}/submit_version", files=files, headers=headers
    )
    assert r.status_code in (
        400,
        422,
...
def test_upload_zip_missing_project_files_e2e(admin_user_data):
    ...
    if r.status_code == 200:
        ...
        if not (
            ...
            print(
                "[WARN] Build logs leer oder keine erwartete Fehlermeldung für fehlende Projektdateien:",
                logs,
            )
    if r.status_code == 200:
        version_id = r.json()["id"]
...
def test_build_multiple_versions_e2e(admin_user_data):
    ...
    logs1 = (
        httpx.get(
            f"{BASE_URL}/projects/{project_id}/versions/{version_id1}/build_logs",
            headers=headers,
        )
        .json()
        .get("build_logs", "")
    ...
    logs2 = (
        httpx.get(
            f"{BASE_URL}/projects/{project_id}/versions/{version_id2}/build_logs",
            headers=headers,
        )
        .json()
        .get("build_logs", "")
