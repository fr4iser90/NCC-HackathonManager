...
from .project import ProjectRead # Assuming schemas/project.py exists and has ProjectRead
...
class HackathonRead(HackathonBase):
    ...
    registrations: List[HackathonRegistrationRead] = [] # Updated to use the new registration schema
...
class ParticipantRegistrationCreate(BaseModel):
    ...
    def check_exclusive_participant(cls, values):
        ...
        if not ((user_id is not None and team_id is None) or \
                (user_id is None and team_id is not None)):
