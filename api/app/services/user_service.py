...
def update_profile(db: Session, user_in: UserUpdate, current_user: User) -> User:
    ...
    if "password" in update_data:
        if not update_data.get("current_password"):
            raise HTTPException(
                status_code=400, detail="Aktuelles Passwort erforderlich."
            )
        if not verify_password(
            update_data["current_password"], current_user.hashed_password
        ):
            raise HTTPException(
                status_code=400, detail="Aktuelles Passwort ist falsch."
            )
...
def upload_avatar_file(db: Session, file: UploadFile, current_user: User) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Fehler beim Speichern des Avatars."
        )
...
def get_my_projects(db: Session, current_user: User):
    ...
    if user_team_ids:
        team_registrations = (
            db.query(HackathonRegistration)
            .filter(HackathonRegistration.team_id.in_(user_team_ids))
            .all()
        )
        for reg in team_registrations:
            project_ids.add(reg.project_id)
    projects = (
        db.query(Project).filter(Project.id.in_(list(project_ids))).all()
        if project_ids
        else []
    )
    return projects
