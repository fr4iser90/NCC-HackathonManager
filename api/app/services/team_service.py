...
def create_team(db: Session, team_in: TeamCreate, current_user: User) -> Team:
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Team name already exists for this hackathon.")
...
def update_team(db: Session, team_id: uuid.UUID, team_in: TeamUpdate) -> Team:
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Team name already exists for this hackathon or other integrity violation.")
...
def accept_join_request(db: Session, team_id: uuid.UUID, user_id: uuid.UUID, current_user: User):
