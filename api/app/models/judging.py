...
from sqlalchemy import Column, String, DateTime, ForeignKey, Enum as SQLEnum, Integer, Text, Numeric, UniqueConstraint
...
class Criterion(Base):
    ...
    weight: Mapped[Decimal] = mapped_column(Numeric, nullable=False, default=1.0) # As per init.sql
...
class Score(Base):
    ...
    __table_args__ = (
        UniqueConstraint('project_id', 'criteria_id', 'judge_id', name='uq_project_criterion_judge'),
        {"schema": "judging"}
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    project_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("projects.projects.id"), nullable=False)
    criteria_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("judging.criteria.id"), nullable=False)
    judge_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("auth.users.id"), nullable=False)
