...
class User(Base):
    ...
    teams: Mapped[List["Team"]] = relationship(
        "Team", secondary="teams.members", back_populates="users", viewonly=True
    )
    ...
    submissions = relationship(
        "Submission", back_populates="submitter", cascade="all, delete-orphan"
    )
    ...
    sent_invites = relationship(
        ...
        foreign_keys="TeamInvite.sender_id",
        cascade="all, delete-orphan",
    )
    received_invites = relationship(
        "TeamInvite",
        back_populates="recipient",
        foreign_keys="TeamInvite.recipient_id",
        cascade="all, delete-orphan",
    )
    sent_join_requests = relationship(
        "JoinRequest",
        back_populates="sender",
        foreign_keys="JoinRequest.sender_id",
        cascade="all, delete-orphan",
    )
    received_join_requests = relationship(
        "JoinRequest",
        back_populates="recipient",
        foreign_keys="JoinRequest.recipient_id",
        cascade="all, delete-orphan",
    )
    project_versions = relationship(
        "ProjectVersion", back_populates="submitter", cascade="all, delete-orphan"
    )
