...
export default function TeamsPage() {
  ...
  // Removed unused joining and leaving state
  ...
  const [myJoinRequests, setMyJoinRequests] = useState<
    Record<string, JoinRequest | null>
  >({});

  type UserWithIdAndAccessToken = { id?: string; accessToken?: string };
  const userId = (session?.user as UserWithIdAndAccessToken)?.id;
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      ...
      setError(null);
      try {
        const token = (session?.user as UserWithIdAndAccessToken)?.accessToken;
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      ...
      const fetchMyJoinRequests = async () => {
        try {
          const token = (session?.user as unknown as { accessToken?: string })
            ?.accessToken;
  ...
  // Removed unused handleJoin and handleLeave
  ...
  // Removed unused myPendingRequest and fetchJoinRequests
  ...
  const handleRevokeRequest = async (teamId: string) => {
    ...
    try {
      ...
      let message = 'Error revoking request.';
      if (
        err &&
        typeof err === 'object' &&
        'response' in err &&
  ...
  const handleAcceptJoin = async (teamId: string, userIdToAccept: string) => {
    try {
      const token = (session?.user as unknown as { accessToken?: string })
        ?.accessToken;
      await axiosInstance.post(
        `/teams/${teamId}/join-requests/${userIdToAccept}/accept`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      setJoinRequests((prev) => ({
        ...prev,
        [teamId]:
          prev[teamId]?.filter((r) => r.user_id !== userIdToAccept) || [],
      }));
    } catch {}
  };
  ...
  const handleRejectJoin = async (teamId: string, userIdToReject: string) => {
    try {
      const token = (session?.user as unknown as { accessToken?: string })
        ?.accessToken;
      await axiosInstance.post(
        `/teams/${teamId}/join-requests/${userIdToReject}/reject`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      setJoinRequests((prev) => ({
        ...prev,
        [teamId]:
          prev[teamId]?.filter((r) => r.user_id !== userIdToReject) || [],
      }));
    } catch {}
  };
          ...
          <div className="relative bg-white rounded-lg shadow-lg p-8 w-full max-w-md z-20">
            ...
            <form onSubmit={handleCreateTeam} className="space-y-4">
              ...
              <div>
                ...
                <select
                  className="mt-1 block w-full border rounded p-2"
                  value={newTeam.is_open ? 'open' : 'closed'}
                  onChange={(e) =>
                    setNewTeam((t) => ({
                      ...t,
      ...
      {loading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {teams.map((team) => {
            ...
            const isOwner =
              team.members?.find((m) => m.user_id === userId)?.role === 'owner';
            return (
              <div
                key={team.id}
              ...
              >
                ...
                <div className="mt-3 flex flex-col gap-2">
                  {!isMember &&
                    (team.is_open ?? true) === true &&
                    !myJoinRequests[team.id] && (
                      <button
                      ...
                      >
                        {requesting === team.id
                          ? 'Requesting...'
                          : requestFeedback[team.id] || 'Request to Join'}
                      </button>
                    )}
                  {!isMember && myJoinRequests[team.id] && (
                    ...
                    >
                      {revoking === team.id
                        ? 'Revoking...'
                        : requestFeedback[team.id] || 'Revoke Request'}
                    </button>
                  )}
                        ...
                        <ul className="space-y-1">
                          {joinRequests[team.id].map((req) => (
                            ...
                            >
                              ...
                              <button
                                className="px-2 py-0.5 bg-green-200 text-green-800 rounded"
                                onClick={() =>
                                  handleAcceptJoin(team.id, req.user_id)
                                }
                              >
                                Accept
                              </button>
                              <button
                                className="px-2 py-0.5 bg-red-200 text-red-800 rounded"
                                onClick={() =>
                                  handleRejectJoin(team.id, req.user_id)
                                }
                              >
                                Reject
                              </button>
                  ...
                  {isMember && !isOwner && (
                    <div className="text-xs text-gray-400">
                      Waiting for owner to manage membership
                    </div>
                  )}
                </div>
              </div>
