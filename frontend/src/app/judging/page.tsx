...
import { Criterion, Score } from '@/types/judging';
...
export default function JudgingDashboard() {
  ...
  const judgeId = (session?.user as UserWithRoleAndAccessToken)?.id;
      ...
      try {
        for (const project of projects) {
          const res = await axiosInstance.get(
            `/judging/scores/project/${project.id}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
          setScores((prev) => ({
            ...prev,
            [project.id]: res.data.filter((s: Score) => s.judge_id === judgeId),
          }));
        }
      } catch {}
    };
  ...
  const openScoring = (project: Project) => {
    ...
    for (const criterion of criteria) {
      const existing = projectScores.find(
  ...
  const handleSubmitScores = async () => {
    ...
    try {
      for (const criterion of criteria) {
        ...
        const existing = (scores[activeProject.id] || []).find(
          (s) => s.criteria_id === criterion.id,
        );
        if (existing) {
          // Update
          await axiosInstance.put(
            `/judging/scores/${existing.id}`,
            {
              score: input.score,
      ...
      // Refetch scores
      const res = await axiosInstance.get(
        `/judging/scores/project/${activeProject.id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      setScores((prev) => ({
        ...prev,
        [activeProject.id]: res.data.filter(
          (s: Score) => s.judge_id === judgeId,
        ),
      }));
    } catch (err: unknown) {
      setFeedback(
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <div
              key={project.id}
              className="bg-white border border-gray-200 rounded-lg shadow p-6 flex flex-col gap-2"
            >
              <div className="font-bold text-lg">{project.name}</div>
              <div className="text-gray-600 text-sm">
                Team: {project.team?.name || project.team_id}
              </div>
              <button
              ...
              <div className="mt-2 text-xs text-gray-500">
                {scores[project.id]?.length
                  ? `${scores[project.id].length} criteria scored`
                  : 'Not yet scored'}
              </div>
            </div>
          ))}
            ...
            {criteria.map((criterion) => (
              <div key={criterion.id} className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInput(criterion.id, 'score', Number(e.target.value))
                  }
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    handleInput(criterion.id, 'comment', e.target.value)
                  }
                  className="mt-1 block w-full border rounded px-2 py-1"
                />
              </div>
