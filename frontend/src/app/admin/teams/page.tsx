    ...
    if (sessionStatus === 'authenticated') {
      ...
      try {
        ...
        if (err.response?.status !== 401) {
          setError(err.response?.data?.detail || err.message || 'Failed to fetch teams.');
        }
      } finally {
        setIsLoading(false);
      }
            ...
            <tbody>
              {teams.map((team) => (
                <tr key={team.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{team.name}</td>
                  ...
                  <td className="px-6 py-4">{new Date(team.updated_at).toLocaleDateString()}</td>
                  <td className="px-6 py-4 text-center">
                    <Link href={`/admin/teams/edit/${team.id}`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-3">
                      Edit
                    </Link>
                    <button 
