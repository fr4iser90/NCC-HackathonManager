...
export default function AdminManageUsersPage() { // Renamed component
  ...
  const fetchUsers = useCallback(async () => {
    if (status === 'authenticated' && (session?.user as any)?.role === 'admin') {
        ...
        // if (!apiBaseUrl) throw new Error("API base URL is not configured."); // REMOVED
        if (!token) throw new Error("Access token is not available.");

        const response = await axiosInstance.get('/users/', { // MODIFIED: Use axiosInstance and relative path
            headers: { 'Authorization': `Bearer ${token}` },
        });
        setUsers(response.data);
        } catch (err: any) {
        console.error("Error fetching users:", err);
        // The interceptor will handle 401, but other errors might still occur
  ...
  const handleDelete = async (userId: string, userEmail: string) => {
    if (window.confirm(`Are you sure you want to delete user ${userEmail}? This action cannot be undone.`)) {
      ...
      try {
        ...
        await axiosInstance.delete(`/users/${userId}`, { // MODIFIED: Use axiosInstance and relative path
          headers: { 'Authorization': `Bearer ${token}` },
        });
  ...
  if (status === 'loading' || isLoading) {
    return <div className="container mx-auto py-8 text-center">Loading user data...</div>; // Changed message
  }
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <h2 className="text-2xl font-semibold mb-4 mt-8">User List</h2> {/* Changed from User Management */}
              ...
              {users.map((user) => (
                <tr key={user.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  ...
                  <td className="px-6 py-4 text-center">
                    <Link href={`/admin/users/edit/${user.id}`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-3">
                      Edit
                    </Link>
                    <button 
