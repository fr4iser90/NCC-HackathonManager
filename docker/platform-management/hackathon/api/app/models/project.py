...
from app.schemas.project import ProjectStatus, ProjectStorageType # Import for model usage
...
class Project(Base):
    ...
    description: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    status: Mapped[ProjectStatus] = mapped_column(SQLEnum(ProjectStatus), nullable=False, default=ProjectStatus.DRAFT)
    
    # Neue Felder für Storage und Deployment
    storage_type: Mapped[ProjectStorageType] = mapped_column(SQLEnum(ProjectStorageType), nullable=False, default=ProjectStorageType.GITHUB)
    ...
    docker_archive_url: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    ...
    last_deploy_date: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
