...
import type { Hackathon, HackathonRegistration } from "../types/hackathon"; 
...
export default function HackathonCard({ hackathon }: { hackathon: Hackathon }) {
  ...
  useEffect(() => {
    ...
    const checkRegistrationStatus = async () => {
      ...
      let foundReg = hackathon.registrations.find(reg => reg.user_id === currentUser?.id);
      ...
      if (!foundReg && hackathon.mode !== 'SOLO_ONLY') {
        ...
        try {
          ...
          if (currentTeams.length === 0 && session) { // Fetch only if logged in and teams not loaded
            ...
            if (teamsRes.ok) {
              ...
              console.error("Failed to fetch user teams for card status check:", teamsRes.statusText);
            }
          }
          
          const userTeamIds = currentTeams.map(t => t.id);
          if (userTeamIds.length > 0) {
            foundReg = hackathon.registrations.find(reg => reg.team_id && userTeamIds.includes(reg.team_id));
          }
        } catch (error) {
  ...
  }, [currentUser, hackathon, apiFetch, session]); // Removed userTeams from dep array to avoid loop if fetch is inside
    ...
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden flex flex-col hover:scale-[1.02] transition-transform h-full">
      ...
      <div className="p-4 flex flex-col gap-2 flex-grow">
        <div className="flex gap-2 flex-wrap items-center">
          {hackathon.category && (
            <span className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{hackathon.category}</span>
          )}
          <span className={`text-xs font-medium px-2.5 py-0.5 rounded 
            ${hackathon.status === "active" ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300" : 
              hackathon.status === "upcoming" ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300" :
              hackathon.status === "completed" ? "bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-300" :
              "bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300"}`}>
            {hackathon.status.toUpperCase()}
          </span>
          {hackathon.tags?.map(tag => (
            <span key={tag} className="bg-slate-200 text-slate-700 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-slate-700 dark:text-slate-300">{tag}</span>
          ))}
        </div>
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">{hackathon.name}</h2>
        <div className="text-xs text-gray-500 dark:text-gray-400">
          {new Date(hackathon.start_date).toLocaleDateString()} - {new Date(hackathon.end_date).toLocaleDateString()}
        </div>
        <div className="text-xs text-gray-500 dark:text-gray-400">Modus: {hackathon.mode.replace(/_/g, " ")}</div>
        {hackathon.sponsor && <div className="text-xs text-gray-500 dark:text-gray-400">Sponsor: {hackathon.sponsor}</div>}
        <p className="text-sm text-gray-700 dark:text-gray-300 mt-1 line-clamp-3 flex-grow">{hackathon.description}</p>
        
        <div className="mt-auto pt-2">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs text-gray-500 dark:text-gray-400">Teilnehmer: {participantCount}</span>
          </div>

          {isLoadingTeams ? (
            <div className="w-full text-center py-2 text-xs text-gray-500 dark:text-gray-400">Checking status...</div>
          ...
          {!isHackathonOpenForRegistration && hackathon.status === "completed" && (
             <button 
           ...
           {!isHackathonOpenForRegistration && hackathon.status === "archived" && (
             <span className="mt-2 block text-center text-xs text-gray-400 dark:text-gray-500 italic">Archiviert</span>
          )}
          <a
