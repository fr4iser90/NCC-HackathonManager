...
export default function SignInPage() {
  ...
  useEffect(() => {
    if (error) {
      // Decode error message if it comes from NextAuth.js (it might be URL encoded)
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const result = await signIn('credentials', {
      ...
      email: email,
      password: password,
      // callbackUrl: callbackUrl, // NextAuth handles callbackUrl internally if redirect is true
    });

    setIsLoading(false);

    if (result?.error) {
      setPageError(result.error === "CredentialsSignin" ? "Invalid email or password." : result.error);
    } else if (result?.ok) {
      // Sign-in was successful
      router.push(callbackUrl); // Redirect to the original target or homepage
    } else {
      // Should not happen if error and ok are correctly handled by NextAuth
